...
<cffunction name="fatalError" output="true">
    ...
    <cfif NOT ( ListFindNoCase('unauthenticated,loggedOut',variables.security.status()) 
        and REFindNoCase('(^login_)|(^warnings)',arguments.targetRoute) gt 0 )>
        <cfset this.warnings = ListAppend(warnings,'warnings_refusedPage')>
    </cfif>
    <cfset route = this.warnings eq ''?arguments.targetRoute:'warnings_home'>
    <cfreturn route> 
</cffunction>    
