...
<cfif (StructKeyExists(form,'username') and form.username neq '') and 
    (StructKeyExists(form,'userpwd') and form.userpwd neq '') >
    
    <!--- hash it first and etc b4 authenticating --->
    <cfset authenticity = request.obj.Security.authenticate(form.username,form.userpwd)>
    <cfdump var=#authenticity#>
</cfif>

<cfif authenticity.flag> 
    <cfset session.data = authenticity.data> 
    <cfset request.redirect='#request.web.url#route=login_home&#session.urltoken#'> 
<cfelse>
